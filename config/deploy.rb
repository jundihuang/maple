...
# ------------------------------------------------------------------------------
  ...
  task :start => :environment do
    ...
    command %{
      cd #{fetch(:deploy_to)}/current && bundle exec unicorn -c config/unicorn.rb -E production -D
    }
  end
#                                                                     Stop task
# ------------------------------------------------------------------------------
  ...
  task :stop do
    ...
    command %{
      test -s "#{fetch(:unicorn_pid)}" && kill -QUIT `cat "#{fetch(:unicorn_pid)}"` && rm -rf "#{fetch(:unicorn_pid)}" && echo "Stop Ok"
      echo >&2 "Not running"
    }
  end
